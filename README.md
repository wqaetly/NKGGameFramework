...
- `GameDebugDumpRecorder`：录制轻量 frame snapshot 与 ECS component store blocks，停止时写出 Odin binary `.nkgdump`，并可在 Web 面板 Timeline 中按帧回放；需要窗口化时可配置 `MaxRecordedDumpFrames`。
