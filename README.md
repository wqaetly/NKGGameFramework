...
- `Gameplay`：引擎无关 GameplayTag、Skill/Buff 运行时，包含 UE 风格层级标签、标签查询、技能书、CD、技能效果、Buff 叠层/刷新/生命周期和 ECS 更新系统。
...
技能效果通过 `SkillEffectRegistry` 扩展；默认 `apply_buff` 效果可以把技能释放连接到 Buff 系统。Buff 效果通过 `BuffEffectRegistry` 扩展，Unity/Godot 特效、动画、材质变化等引擎相关行为应在 Adapter 或业务层注册，不进入主包。
