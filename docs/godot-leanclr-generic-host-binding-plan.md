...
- `NkgLeanClrPlaneHost`：Godot 场景 root，保留样例动作绑定、host/debug 上下文同步和 smoke-only 状态访问器。
...
- 已完成部分：command buffer / native reader / `NkgGodotHost` 支持 `CreateNode`、`DestroyObject`、`SetParent`、`SetTransform2D`、`SetVisible`、`SetProperty`、`CallMethod`、`LoadResource`、`ReleaseResource`、`InstantiateScene` 的最小通路。
...
- 待完成：扩大 command set，支持更多 Godot object/resource 操作和更强错误诊断。
- 支持批量提交和主线程 flush。
...
- 支持 `InstantiateScene(path)`。已完成基于 `PackedScene` resource handle 的最小 `InstantiateScene` 命令。
...
```text
把 NkgLeanClrPlaneHost 剩余的 status 边界继续拆小，
